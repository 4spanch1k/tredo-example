...
const GENERIC_NEWS_QUESTION =
  /что\s+вы\s+проверяете\s+первым,\s+прежде\s+чем\s+менять\s+привычный\s+процесс/iu;
...
export function assertNewsBackedPost(text: string, newsItem: NewsItem): void {
  if (GENERIC_NEWS_QUESTION.test(text)) {
    throw new Error("Generated news post uses a generic question instead of the article context");
  }
  const normalizedText = text.toLocaleLowerCase("ru");
  const normalizedSource = newsItem.source_name.trim().toLocaleLowerCase("ru");
  if (normalizedSource && normalizedText.includes(normalizedSource)) {
    throw new Error("Generated news post exposes the source name instead of presenting the news");
  }
  ...
  if (!hasMaterialAnchor) {
    throw new Error("Generated news post does not contain a material anchor");
  }
}
...
export const POST_GENERATION_SYSTEM_PROMPT = [
  ...
  "Если в запросе есть проверенный материал для IT-повестки, используй только его заголовок и краткое содержание. Источник нужен только для внутренней проверки: не упоминай название источника и не используй конструкции вроде «Habr сообщает». Оставь в тексте узнаваемую деталь самого материала, чтобы пост нельзя было принять за общую заготовку. Вопрос должен быть связан именно с событием: обновлением, AI-инструментом, риском, рынком или последствием для пользователя. Запрещён универсальный вопрос «Что вы проверяете первым, прежде чем менять привычный процесс?». Не добавляй неподтверждённые детали, даты, цифры и оценки, не копируй заголовок дословно и не вставляй ссылку в пост. Переводи новость в один практический вопрос, особенно о том, как люди используют AI или голосовых агентов.",
