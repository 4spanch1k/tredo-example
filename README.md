...
Генератор держит заполненными ближайшие 26 часов и добавляет не больше трёх слотов за запуск. Основной автор постов — Gemini 3.6 Flash; если его лимит исчерпан или вариант не прошёл проверку, генерацию подхватывает Groq. В prompt попадает только короткая свежая выборка, а проверка точных повторов отдельно сверяется с историей до 1000 публикаций. Точный, смысловой, несвязный или структурно однотипный вариант отклоняется; база дополнительно запрещает одинаковый текст среди запланированных и опубликованных записей. Кратковременный лимит обеих моделей не удаляет слот: Cron повторяет заполнение очереди в следующем цикле.
