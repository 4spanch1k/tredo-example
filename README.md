...
| Сценарий | Поведение |
| --- | --- |
| Комментарий под своим постом | Собирает ограниченный контекст ветки и готовит короткий уместный ответ без выдуманных фактов. |
...
Если Meta не доставила webhook, `interaction-poller` раз в пять минут читает полные ветки ответов к последним 24 собственным постам — это двое суток при текущем расписании — и до 50 упоминаний через официальный Threads API. Поэтому он видит не только первый комментарий, но и продолжение диалога. Оба пути используют один ключ дедупликации:
...
sequenceDiagram
    ...
    else Обычный комментарий или спокойная критика по теме поста
        P->>T: Короткая человеческая реплика без продажи
    else Не хватает контекста, ответа или модель перегружена
        P->>O: Telegram: автоответ остановлен и нужна помощь
        P->>D: Ничего не придумывает
    end
```
...
- В обычных слотах публикации вращаются между десятью рубриками: клиентский сервис, удобство сайта, доверие к бизнесу, работа над digital-проектом, автоматизация рутины, мобильные продукты, внутренние процессы бизнеса, голосовые AI-агенты, AI-инструменты и скрытые сбои автоматизации.
- Два отдельных слота каждый день посвящены подтверждённым IT-новостям из Habr: 12:00 и 18:00 по Алматы. Один слот в 20:00 отведён мягкому рекламному посту; услуга в нём меняется, но продажа не появляется в каждом посте.
- Девять остальных слотов берут разные практические рубрики; пропущенная десятая рубрика сдвигается на следующий день, поэтому одинаковые темы не идут подряд. Если текст ушёл от назначенной темы к привычному разговору про кнопку, цену или форму, вариант отклоняется.
- Голосовая тема разбирает реальные точки провала: перебивание, потерю контекста, паузы, акценты, уверенные ошибки и момент передачи человеку. AI- и IT-тема задаёт практический вопрос, а не повторяет рекламный анонс.
- IT-новостные слоты используют только записи из `it_news_items`, которые собрал `it-news-radar` из настроенного RSS/Atom-источника. В публичном тексте источник не называется: пост выглядит как короткая новость с узнаваемой деталью события и одним практическим вопросом. Без источника слот остаётся общим вопросом о практике; свежие факты, даты и цифры не выдумываются.
- Все фразы описывают одного героя и одну проблему; финальный вопрос обязан прямо продолжать предыдущую мысль. Модель возвращает отдельную смысловую самопроверку, иначе вариант отклоняется.
- Длина — от 45 до 240 символов: две или три короткие фразы без профессионального жаргона.
- Из каждых 12 публикаций ровно одна, в слоте 20:00 по Алматы, мягко продаёт одну услугу: сайт, лендинг, приложение, AI-агента или автоматизацию. Остальные 11 не содержат предложения агентства.
...
Явный коммерческий запрос или конкретный вопрос об услуге, цене, сроке или связи получает ответ по задаче и переход в WhatsApp. Шутки, поддержка, обычные мнения и спокойная критика по теме поста получают короткую человеческую реплику без продажи и ссылки.

Перед ответом бот берёт только текущую ветку: исходный пост и до восьми сообщений, максимум 2400 символов. Если родительский комментарий не найден, ветка длиннее лимита, модель недоступна или вопрос требует неизвестного факта, цены, срока либо кейса, бот ничего не публикует и уведомляет оператора в Telegram. Агрессия, жалобы, юридические вопросы, персональные данные и спам также не получают автоматический ответ.

Планируется 12 слотов ежедневно по времени Алматы (UTC+5): каждые два часа с **00:00** до **22:00**. В базе это нечётные часы UTC: `01:00`, `03:00`, …, `23:00`. Генератор каждые 15 минут заполняет будущие слоты и при восстановлении добавляет только один последний пропущенный слот за два часа, чтобы не публиковать накопившуюся пачку. Poster каждые 5 минут забирает уже наступившие публикации и готовые повторные попытки.

Генератор держит заполненными ближайшие 26 часов и добавляет не больше трёх слотов за запуск. Посты пишет Gemini 3.6 Flash, а лимит Groq полностью зарезервирован для классификации комментариев и человеческих ответов. В prompt попадает только короткая свежая выборка, а проверка точных повторов отдельно сверяется с историей до 1000 публикаций. Точный, смысловой, несвязный или структурно однотипный вариант отклоняется; база дополнительно запрещает одинаковый текст среди запланированных и опубликованных записей. Если Gemini временно недоступна, Cron повторяет заполнение очереди в следующем цикле.
...
GEMINI_API_KEY=<ключ Gemini для генерации постов>
...
npx supabase functions deploy it-news-radar --no-verify-jwt
```

Для ежедневной IT-повестки по умолчанию используется официальный RSS-раздел новостей Habr. При необходимости источник можно переопределить в `supabase/.env.functions`:

```bash
IT_NEWS_FEED_URL=https://habr.com/ru/rss/news/?fl=ru&limit=100
IT_NEWS_SOURCE_NAME=Habr
```

Функция сохраняет только заголовок, ссылку, краткое описание и дату публикации; исходный XML не сохраняется. После настройки повторно примените `supabase/cron_setup.sql`: сбор запускается ежедневно в 11:07 по Алматы.
...
| Content Generator | `*/15 * * * *` | Пополнение ближайших слотов без накопления большой очереди. |
| Content Poster | `*/5 * * * *` | Публикация наступивших слотов и готовых повторных попыток. |
| Keyword Radar | `23 */3 * * *` | Поиск коммерческих публикаций и операторский алерт. |
| IT News Radar | `7 6 * * *` | Ежедневный сбор новостей из Habr в `it_news_items`; это 11:07 по Алматы. |
