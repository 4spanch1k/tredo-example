...
ENGAGEMENT_REPLY_BLOCKED = re.compile(
    ...
    r"благодарим|рады,\s+что|обращайтесь|это\s+хороший\s+вопрос|зависит\s+от\s+того|"
    r"интересн\w*\s+(?:мысль|точка\s+зрения|классификация|вопрос)|"
    r"(?:удобство|ясность|честность|спокойствие)\w*.{0,50}(?:важн|лучше)|"
    r"действительно\s+(?:важн|многое)|как\s+можно\s+было\s+бы\s+улучшить|"
    r"всегда\s+приятно|автоматизация\s+может|тонкий\s+момент|просто\s+обсуждаем)",
    re.IGNORECASE,
)
